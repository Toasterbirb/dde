...
applyrules(Client *c)
{
	const char *classifier, *instance;
	unsigned int i;
	const Rule *r;
	Monitor *m;
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->classifier || strstr(classifier, r->classifier))
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - sw, 0, sw + lrpad / 2, bh + lrpad / 2 - barbottom + bartextheightoffset, lrpad / 2, stext, 0);

		drw_setscheme(drw, scheme[SchemeNormShadow]);
		if (enable3dbar)
			drw_rect(drw, m->ww - sw, lrpad + 6, sw + lrpad / 2, barbottom + bartextheightoffset, 1, 0); /* add shadow bar for status */
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]); /* Get text (font) width of the current tag and use that as tag width */
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm]);

		/* lrpad / 2 centers the tag text */
		drw_text(drw, x, 0, w, bh + lrpad / 2 - barbottom + bartextheightoffset, lrpad / 2, tags[i], urg & 1 << i); /* Add tag names from 1 to 9 */
	...
	x = drw_text(drw, x, 0, w, bh + lrpad / 2 - barbottom + bartextheightoffset, lrpad / 2, m->ltsymbol, 0); /* Active tiling mode */
	...
	if ((w = m->ww - sw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w, bh + lrpad / 2 - barbottom + bartextheightoffset, lrpad / 2, m->sel->name, 0); /* Print the title of selected window */
...
manage(Window w, XWindowAttributes *wa)
{
	...
	XSetWindowBorder(dpy, w + 50, scheme[SchemeSel][ColBorder].pixel);
